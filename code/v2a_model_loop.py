...
class V2AModel(pl.LightningModule):
    def __init__(self, opt, betas_path) -> None:
        ...
        try:
            self.pose_opt_interval = opt.model.pose_opt_interval
            print(f"pose_opt_interval {self.pose_opt_interval}")
        except:
            print("#####attention: pose_opt_interval is not defined, set to 10 by default#####")
            self.pose_opt_interval = 10
        try:
            self.pose_opt_epoch = opt.model.pose_opt_epoch
            print(f"pose_opt_epoch {self.pose_opt_epoch}")
        except:
            print("#####attention: pose_opt_epoch is not defined, set to 1 by default#####")
            self.pose_opt_epoch = 1
        try:
            self.use_loop = opt.model.use_loop
            print(f"use_loop {self.use_loop}")
        except:
            print("#####attention: use_loop is not defined, set to True by default#####")
            self.use_loop = True
        try:
            self.nerfacc = opt.model.use_nerfacc
            print(f"use_nerfacc {self.nerfacc}")
        except:
            print("#####attention: use_nerfacc is not defined, set to False by default#####")
            self.nerfacc = False
    ...
    def training_step(self, batch):
        ...
        if self.using_sam:
            ...
            is_delayed_opt = self.current_epoch < self.pose_correction_epoch and not is_certain
            # if self.current_epoch < 500 and batch_idx > 85 and batch_idx < 125: # this is for warmwelcome
            # TODO, for first several epoch, pose should not be updated, pose should use average avatar
            if self.use_loop:
                # if is_delayed_opt or is_pose_opt:
                #     # print("pose opt")
                #     cur_opt = opt_pose
                #     opt_idx = 1
        ...
        if (is_delayed_opt or self.all_edge) and 'edge_uv' in inputs.keys():
    ...
    def get_depth_order_loss(self, inputs):
        ...
        for person_idx, smpl_server in enumerate(self.model.smpl_server_list):
            ...
            # deformed_verts = smpl_output['smpl_verts']
    ...
    def test_step_each_person_canonical(self, batch, id, prefix='canonical'):
        ...
        for i in range(num_splits):
            ...
            if self.opt_smpl:
                ...
                # batch_inputs['smpl_trans'] = torch.zeros_like(batch_inputs['smpl_trans'])
                # batch_inputs['smpl_pose'] = torch.zeros_like(batch_inputs['smpl_pose'])
    ...
    def test_step(self, batch, *args, **kwargs):
        ...
        # if self.num_person > 1:
        #     for i in range(self.num_person):
        #         self.test_step_each_person_canonical(batch, i, prefix='canonical')
