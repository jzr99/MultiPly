...
image_writer = imageio.get_writer(os.path.join(seq_dir, 'ep2_1.mp4'), fps=30, macro_block_size=1)
overlay_normal_writer = imageio.get_writer(os.path.join(seq_dir, 'ep2_2.mp4'), fps=30, macro_block_size=1)
# rotate_normal_writer = imageio.get_writer(os.path.join(seq_dir, 'ep1_3.mp4'), fps=30, macro_block_size=1)
normal_writer = imageio.get_writer(os.path.join(seq_dir, 'ep2_3.mp4'), fps=30, macro_block_size=1)
for idx in range(len(image_paths)):
    ...
    if idx >= ep3_frame:
        ...
        mask = (imageio.imread(mask_path) > 127)[:, :, np.newaxis]
        overlay_normal = (imageio.imread(overlay_normal_path))
        masked_normal = ((overlay_normal * mask) + (np.ones_like(overlay_normal) * 255 * (1 - mask))).astype(np.uint8)
        normal_writer.append_data(masked_normal)
    # if idx == ep2_frame:
    #     overlay_normal_path = overlay_normal_paths[idx]
