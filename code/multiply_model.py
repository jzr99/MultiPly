...
class MultiplyModel(pl.LightningModule):
    ...
    def init_params(self, opt):
        ...
        try:
            self.depth_epoch = opt.model.depth_epoch
            print(f"depth_epoch {self.depth_epoch}")
        except:
            self.depth_epoch = [200, 1000]
            print(f"depth_epoch {self.depth_epoch}")
        try:
    ...
    def training_step(self, batch):
        ...
        is_pose_depth_opt = 'sam_mask' in inputs.keys() and self.current_epoch >= 200 and pose_epoch < self.pose_opt_epoch and self.current_epoch < 1000 and not self.depth_end
        ...
        if self.using_sam:
            ...
            if self.use_loop:
                # optimze pose with depth loss (default disable)
