...
from ..utils.snarf_utils import hierarchical_softmax, bmv, create_voxel_grid, query_weights_smpl
...
class ForwardDeformer(torch.nn.Module):
    ...
    def __init__(self, opt, smpl_server=None):
        ...
        if self.opt.skinning_mode == 'preset':
            ...
            self.lbs_network = ImplicitNet(self.opt.network)
    ...
    def forward(self, xd, cond, tfs, eval_mode=False):
        ...
        if eval_mode or False: # or self.opt.use_smpl_weights:
        ...
        if not False: # self.opt.use_slow_grad:
