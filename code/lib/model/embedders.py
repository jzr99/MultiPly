...
class HannWindowEmbedder:
    ...
    def create_embedding_fn(self):
        ...
        kick_in_epoch = torch.tensor(self.kwargs['kick_in_epoch'],
                                    dtype=torch.float32)
        t = torch.clamp(self.kwargs['iter_epoch'] - kick_in_epoch, min=0.)
...
def get_embedder(multires, input_dims=3, mode='fourier', kick_in_epoch=None, iter_epoch=None, full_band_epoch=None):
    embed_kwargs = {
        'include_input': True,
