...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            surface_canonical_points = canonical_points.reshape(num_pixels, N_samples, 3)[surface_mask]

            surface_ray_dirs = dirs[surface_mask]
            surface_cam_loc = cam_loc.unsqueeze(1).repeat(1, N_samples, 1)[surface_mask]
            ...
            surface_canonical_points = surface_canonical_points.reshape(-1, 3)
            surface_ray_dirs = surface_ray_dirs.reshape(-1, 3)
            surface_cam_loc = surface_cam_loc.reshape(-1, 3)
            ...
            differentiable_surface_points = canonical_points.reshape(num_pixels, N_samples, 3)[surface_mask].reshape(-1, 3)
            grad_theta = None

        sdf_output = sdf_output.reshape(num_pixels, N_samples, 1)[surface_mask].reshape(-1, 1)
        z_vals = z_vals[surface_mask]
        view = -dirs[surface_mask].reshape(-1, 3) # view = -ray_dirs[surface_mask]
        ...
        if differentiable_surface_points.shape[0] > 0:
            if self.use_body_pasing:
                rgb_values, others = self.get_rbg_value(differentiable_surface_points, view,
                                                                      cond, smpl_tfs, surface_body_parsing=surface_body_parsing, is_training=self.training)
            else:
                rgb_values, others = self.get_rbg_value(differentiable_surface_points, view,
                                                                      cond, smpl_tfs, is_training=self.training)                       
            normal_values = others['normals'] # should we flip the normals?
        ...
        if self.training:
            if differentiable_surface_points.shape[0] > 0:
                normal_weight = torch.einsum('ij, ij->i', normal_values, -ray_dirs[surface_mask]).detach()
            output = {
                'points': points,
            ...
            output = {
                'points': points,
                'rgb_values': rgb_values_global,
                'normal_values': normal_values_global,
                'sdf_output': sdf_output,
                # 'network_object_mask': network_object_mask,
                # 'object_mask': object_mask
            }
