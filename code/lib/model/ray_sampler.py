...
class ErrorBoundSampler(RaySampler):
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model): # def get_z_vals(self, ray_dirs, cam_loc, model, cond, smpl_tfs, eval_mode): def get_z_vals(self, ray_dirs, cam_loc, model):
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            with torch.no_grad():
                samples_sdf = model.implicit_network.get_sdf_vals(points_flat) # model.sdf_func(points_flat, cond, smpl_tfs, eval_mode)[0] model.implicit_network.get_sdf_vals(points_flat)
