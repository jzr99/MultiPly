...
class BaseFitter(object):
    ...
    def save_outputs(self, save_paths, scan_paths, smpl, th_scan_meshes, save_name='smpl'):
        th_smpl_meshes = self.smpl2meshes(smpl)
        mesh_paths, names = self.get_mesh_paths(save_name, save_paths, scan_paths)
        ...
        # Save params
        self.save_smpl_params(names, save_paths, smpl, save_name)
    ...
    def get_mesh_paths(self, save_name, save_paths, scan_paths):
        names = [split(s)[1] for s in scan_paths]
        # Save meshes
        mesh_paths = []

        if type(save_paths) is list:
            for n, p in zip(names, save_paths):
                if n.endswith('.obj'):
                    mesh_paths.append(join(p, n.replace('.obj', f'_{save_name}.ply')))
                else:
                    mesh_paths.append(join(p, n.replace('.ply', f'_{save_name}.ply')))
        else:
            for n in names:
                if n.endswith('.obj'):
                    mesh_paths.append(join(save_paths, n.replace('.obj', f'_{save_name}.ply')))
                else:
                    mesh_paths.append(join(save_paths, n.replace('.ply', f'_{save_name}.ply')))

        return mesh_paths, names

    def save_smpl_params(self, names, save_paths, smpl, save_name):
        if type(save_paths) is not list:
            save_paths = [save_paths]*len(names)
        for p, b, t, n, s in zip(smpl.pose.cpu().detach().numpy(), smpl.betas.cpu().detach().numpy(),
                              smpl.trans.cpu().detach().numpy(), names, save_paths):
            smpl_dict = {'pose': p, 'betas': b, 'trans': t}
            sfx = splitext(n)[1]
            pkl_file = join(s, n.replace(sfx, f'_{save_name}.pkl'))
