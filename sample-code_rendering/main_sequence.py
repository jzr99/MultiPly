...
if __name__ == "__main__":
    ...
    skip_step = 2
    normalize_trans = 0.
    for idx, file_path in enumerate(files[::skip_step]):
        ...
        if idx == 0:
            ...
            normalize_trans = -(max + min) / 2
            raw_scan.vertices = (raw_scan.vertices + normalize_trans) # / (max - min).max() * 2
        else:
            raw_scan.vertices = (raw_scan.vertices + normalize_trans)
        meshes = [raw_scan]
        ...
        for i, (render_img, render_depth) in enumerate(zip(render_imgs, render_depths)):
            ...
            os.makedirs(f"outputs/image_cam{i:02d}", exist_ok=True)
            os.makedirs(f"outputs/mask_cam{i:02d}", exist_ok=True)
            cv2.imwrite(f"outputs/image_cam{i:02d}/image_{frame:04d}.png", render_img)
            cv2.imwrite(f"outputs/mask_cam{i:02d}/mask_{frame:04d}.png", render_mask)
    ...
    np.save("outputs/normalize_trans.npy", np.array(normalize_trans))