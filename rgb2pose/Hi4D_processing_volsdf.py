...
if dataset == 'Hi4D':
    ...
    seq_dir = f'{DIR}/smpl'
    mask_dir = f'{DIR}/seg/img_seg_mask/{camera_view}/{person_index}'
    save_dir = '/Users/jiangzeren/Downloads/eval/V2A'
...
if dataset == "Hi4D":
    smpl_shape = np.load("/Users/jiangzeren/Downloads/eval/data/pair01/hug01/smpl/000006.npz", allow_pickle=True)['betas'][person_index]
else:
...
for idx, img_path in enumerate(tqdm(img_paths)):
    ...
    if dataset =="Hi4D":
        smpl_pose = np.concatenate([seq_file['global_orient'][person_index],seq_file['body_pose'][person_index]], axis=0)
        smpl_trans = seq_file['transl'][person_index]
    else:
...
smpl_verts = smpl_output.vertices.data.cpu().numpy().squeeze()
_ = trimesh.Trimesh(smpl_verts).export('/Users/jiangzeren/Downloads/eval/V2A/test.ply')
...
# ipdb.set_trace()
cv2.imwrite('/Users/jiangzeren/Downloads/eval/V2A/test.png', output_img)
