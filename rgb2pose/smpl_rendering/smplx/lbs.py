...
def lbs(
    ...
    # homogen_coord_normals = torch.zeros([batch_size, v_posed.shape[1], 1],
    #                                      dtype=dtype, device=device)
    # normals_homo = torch.cat([normals, homogen_coord_normals], dim=2)  
    # normals_posed_homo = torch.matmul(T, torch.unsqueeze(normals_homo, dim=-1))               
    # import pdb
    # pdb.set_trace()
    # normals_posed = normals_posed_homo[:, :, :3, 0]

    verts = v_homo[:, :, :3, 0]

    return verts, J_transformed # , normals_posed
