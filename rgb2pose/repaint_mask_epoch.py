...
epoch_list = list(range(0, 1500, 50))
for epoch in epoch_list:
    ...
    sam_mask_path = os.path.join(result_dir, f'stage_sam_mask/{epoch:05d}/sam_opt_mask.npy')
    # sam_mask_path = os.path.join(result_dir, 'stage_sam_mask/00000/sam_opt_mask.npy')
    ...
    color = [[30 / 255, 144 / 255, 255 / 255, 0.9], [1.0, 0.749, 0/412, 0.9]]
    for idx, image_path in enumerate(tqdm(image_paths)):
        if idx != 35:
            continue
        save_path_sam = os.path.join(result_dir, 'figure_sam_mask', f'{idx:03d}')
        if epoch == 0:
            os.makedirs(save_path_sam, exist_ok=True)
        # P,
        # if idx == 39:
        ...
        for p in range(sam_mask_idx.shape[0]):
            if p==0:
                show_mask(sam_mask_idx[p] > -.0, plt.gca(), color=color[p])
            else:
                show_mask(sam_mask_idx[p] > -.0, plt.gca(), color=color[p])
        # show_box(bounding_box, plt.gca())
        # show_points(input_point, input_label, plt.gca(), marker_size=30)
        plt.axis('off')
        plt.savefig(os.path.join(save_path_sam, '%05d.png' % epoch), bbox_inches='tight', pad_inches=0.0)
        plt.close()
