...
result_dir = '/media/ubuntu/hdd/easymocap/EasyMocap/3rdparty/Self-Correction-Human-Parsing/data/pair15_fight15_vitpose_4/mask-schp/4'
data_dir = f'/media/ubuntu/hdd/RGB-PINA/data/pair15_fight15_vitpose_4'
...
save_path_sam = os.path.join(result_dir, 'figure_schp_mask')
...
for idx, image_path in enumerate(tqdm(image_paths)):
    ...
    if idx == 51:
        schp_path_1 = os.path.join(result_dir, f'{idx:06d}_0.png')
        schp_path_2 = os.path.join(result_dir, f'{idx:06d}_1.png')
        # read schp mask
        schp_mask_1 = cv2.imread(schp_path_1)
        schp_mask_2 = cv2.imread(schp_path_2)
        # to gray
        schp_mask_1 = cv2.cvtColor(schp_mask_1, cv2.COLOR_BGR2GRAY)
        schp_mask_2 = cv2.cvtColor(schp_mask_2, cv2.COLOR_BGR2GRAY)
        # to binary
        schp_mask_1 = (schp_mask_1 / 255.0) > 0.5
        schp_mask_2 = (schp_mask_2 / 255.0) > 0.5
        ...
        plt.imshow(image)
        show_mask(schp_mask_1, plt.gca(), color=color[1])
        show_mask(schp_mask_2, plt.gca(), color=color[0])
