...
def main(args):
    ...
    for i in range(number_person):
        smpl_model_list.append(SMPL('../code/lib/smpl/smpl_model', gender="MALE").to(device))
    ...
    for idx, img_path in enumerate(tqdm(img_paths)):
        ...
        # if args.mode == 'mask' or args.mode == 'refine':
            ...
            cost_matrix = np.zeros((2, len(seq_file['smpl_thetas'])))
            for person_i in range(2):
                if idx == 0:
                    last_j3d.append(seq_file['joints'][actor_order])
                    last_pj2d.append(seq_file['pj2d_org'][actor_order])
                    last_cam_trans.append(seq_file['cam_trans'][actor_order])
                for i in range(len(seq_file['smpl_thetas'])):
                    ...
                    cost_matrix[person_i, i] = np.linalg.norm(seq_file['cam_trans'][i] + seq_file['joints'][i].mean(0) - last_j3d[-1][person_i].mean(0, keepdims=True) - last_cam_trans[-1][person_i]) # 3D
            ...
            for person_i in range(2):
                ...
                if len(seq_file['smpl_thetas']) >= 2:
                    ...
                    cam_trans = seq_file['cam_trans'][actor_id]

                # undetected person
                if len(seq_file['smpl_thetas']) == 1:
                    # TODO
                    dist = []
                    for i in range(2):
    ...
    if args.mode == 'final':
        ...
        np.save(os.path.join(save_dir, 'gender.npy'), np.array(['male', 'male']))
